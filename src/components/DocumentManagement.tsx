...
export const DocumentManagement = () => {
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      setError(null);
      const response = await axios.get('http://192.168.30.4:5000/list_documents');
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post('http://192.168.30.4:5000/add_document', formData, {
        headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`http://192.168.30.4:5000/delete_document/${documentToDelete}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
